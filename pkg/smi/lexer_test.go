...
func TestLexer_Example(t *testing.T) {
	expected := []struct {
		...
		{tok: tOBJECT_IDENTIFIER, id: "a"},
		{tok: tOBJECT_IDENTIFIER, id: "foo"},
		{tok: tNEGATIVENUMBER, integer32: -42},
		{tok: tOBJECT_IDENTIFIER, id: "as-df"},
		{tok: tOBJECT_IDENTIFIER, id: "boo"},
		{tok: '-'},
		{tok: '-'},
		{tok: tOBJECT_IDENTIFIER, id: "MOO"},
		{tok: tOBJECT_IDENTIFIER, id: "FASDFASD"},
		{tok: tQUOTED_STRING, text: "is quoted!"},
		{tok: tNUMBER, unsigned32: 42},
		{tok: tOBJECT_IDENTIFIER, id: "bar"},
	}
...
func TestLexer_ident(t *testing.T) {
	...
	value := &smiSymType{}
	token := lex.Lex(value)
	if token != tOBJECT_IDENTIFIER {
		t.Errorf("expected %d, got %d", tOBJECT_IDENTIFIER, token)
	}
	if value.id != "foo" {
...
func TestLexer_comment(t *testing.T) {
	testCases := []commentTC{
		...
		{"-- test\n", lexEOF, ""},
		{"-- one\n-- two", lexEOF, ""},
		{"one -- test\n", tOBJECT_IDENTIFIER, "one"},
		{"two --test--\n", tOBJECT_IDENTIFIER, "two"},
		{"three--test", tOBJECT_IDENTIFIER, "three"},
		{"--test--four", tOBJECT_IDENTIFIER, "four"},
	}
